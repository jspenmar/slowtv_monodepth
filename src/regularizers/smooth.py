...
def compute_grad(x: ty.T, /, use_blur: bool = False, ch_mean: bool = False) -> tuple[ty.T, ty.T]:
    """Compute absolute spatial gradients in `(x, y)` directions.

    :param x: (Tensor) (b, c, h, w) Input tensor.
    ...
    if ch_mean: dx, dy = dx.mean(dim=1, keepdim=True), dy.mean(dim=1, keepdim=True)
    return dx, dy


def compute_laplacian(x: ty.T, /, use_blur: bool = False, ch_mean: bool = False) -> tuple[ty.T, ty.T, ty.T, ty.T]:
    ...
    """
    dx, dy = compute_grad(x, use_blur=use_blur)
    dxx, dxy = compute_grad(dx, use_blur=use_blur)
    dyx, dyy = compute_grad(dy, use_blur=use_blur)
...
class SmoothReg(nn.Module):
    ...
    def forward(self, disp: ty.T, img: ty.T) -> ty.LossData:
        ...
        :return: (
            ...
            loss_dict: (TensorDict) {
                'disp_grad': (Tensor) (b, 1, h, w) Average disparity spatial gradients.
                'image_grad': (Tensor) (b, 1, h, w) Average image spatial gradients.
            }
        )
        """
        ...
        if self.use_edges:
            # In-place breaks `torch.compile`
            disp_dx = disp_dx*(-img_dx).exp()
            disp_dy = disp_dy*(-img_dy).exp()
...
class FeatPeakReg(nn.Module):
    ...
    def forward(self, feat: ty.T, img: ty.T) -> ty.LossData:
        ...
        :return: (
            loss: (Tensor) (,) Loss value,
            loss_dict: (TensorDict) {
                'feat_grad': (Tensor) (b, 1, h, w) Average feature spatial gradients.
            }
        )
        """
        ...
        if self.use_edges:
            ...
            feat_dx = feat_dx*(-dx).exp()
            feat_dy = feat_dy*(-dy).exp()
...
class FeatSmoothReg(nn.Module):
    ...
    def forward(self, feat: ty.T, img: ty.T) -> ty.LossData:
        ...
        if self.edge_aware:
            ...
            feat_dxx = feat_dxx*(-dxx).exp()
            feat_dyy = feat_dyy*(-dyy).exp()
            feat_dxy = feat_dxy*(-dxy).exp()
            feat_dyx = feat_dyx*(-dyx).exp()
