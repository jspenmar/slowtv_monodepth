...
class DenseL1Error(nn.Module):
    ...
    def forward(self, pred: ty.T, target: ty.T) -> ty.T:
...
class PhotoError(nn.Module):
    ...
    def __init__(self, weight_ssim: float = 0.85):
        super().__init__()
        if not (0 <= weight_ssim <= 1): raise ValueError(f'Invalid SSIM weight. ({weight_ssim} vs. [0, 1])')
    ...
    def forward(self, pred: ty.T, target: ty.T) -> ty.T:
        ...
        if self.ssim: loss += self.weight_ssim * self.ssim(pred, target).mean(dim=1, keepdim=True)
        if self.l1: loss += self.weight_l1 * self.l1(pred, target)
